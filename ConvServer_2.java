...
public class ConvServer_2 {
	...
	public static void process(Socket clientSocket) throws IOException {
		...
		if (paras[0].equals("m") && paras[1].equals("cm")) {
			System.out.println(Double.parseDouble(paras[2]) * 100);
			out.println(Double.parseDouble(paras[2]) * 100);
		} else if (paras[0].equals("cm") && paras[1].equals("m")) {
			System.out.println(Double.parseDouble(paras[2]) / 100);
			out.println(Double.parseDouble(paras[2]) / 100);
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			out.println("set cm m baobaoioz.koding.io "+port);
			// close IO streams, then socket
			out.close();
			in.close();
			notifySocket.close();
