...
public class ConvServer_1 {
	...
	public static void process(Socket clientSocket) throws IOException {
		...
		if (paras[0].equals("in") && paras[1].equals("cm")) {
			System.out.println(Double.parseDouble(paras[2]) * 2.54);
			out.println(Double.parseDouble(paras[2]) * 2.54);
		} else if (paras[0].equals("cm") && paras[1].equals("in")) {
			System.out.println(Double.parseDouble(paras[2]) / 2.54);
			out.println(Double.parseDouble(paras[2]) / 2.54);
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			out.println("set in cm baobaoioz.koding.io "+port);
			// close IO streams, then socket
			out.close();
			in.close();
			notifySocket.close();
